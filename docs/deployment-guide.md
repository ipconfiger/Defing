...
> 配套材料：[docs/defing-cluster.md](defing-cluster.md)（容器化集群踩坑记录）、[docs/research-cluster-bootstrap.md](research-cluster-bootstrap.md)（静态成员表建群设计）。
...
4. 详细踩坑见 [docs/defing-cluster.md](defing-cluster.md)。
...
`docs/docker-compose.yml.demo`（基于已构建镜像的变体）：
...
| `docs/docker-compose.yml.demo` | 基于已构建镜像的集群 compose 变体 |
| `docs/defing-cluster.md` | 容器化集群部署踩坑记录（坑 A/B/C 系列） |
