...
use dsh_core::{ErrorKind, StateMachine, InMemoryStore, token_hash};
...
fn create_pa(s: &mut StateMachine, project: &str, username: &str) {
    s.apply(&Command::ProjectAdminCreate {
...
fn pa_create_and_get() {
    ...
    let acct = s.get_project_admin("alice").unwrap().expect("account exists");
...
fn pa_create_duplicate_rejected() {
    let mut s = setup_project("alpha");
    create_pa(&mut s, "alpha", "alice");
    let err = s.apply(&Command::ProjectAdminCreate {
...
fn pa_delete_cascades_session() {
    ...
    s.apply(&Command::PaSessionLogin {
        username: "alice".to_string(),
    ...
    s.apply(&Command::ProjectAdminDelete { username: "alice".to_string() }, 3_000).unwrap();
    assert!(s.get_project_admin("alice").unwrap().is_none());
    assert!(s.get_pa_session("alice").unwrap().is_none(), "删号必须级联删会话");

    let err = s.apply(&Command::ProjectAdminDelete { username: "alice".to_string() }, 4_000).unwrap_err();
    assert_eq!(err.kind, ErrorKind::NotFound);
}
...
fn pa_set_password_cascades_session() {
    ...
    s.apply(&Command::ProjectAdminSetPassword {
        username: "alice".to_string(),
        salt: "new-salt".to_string(),
        password_hash: "new-hash".to_string(),
    }, 3_000).unwrap();
    assert!(s.get_pa_session("alice").unwrap().is_none(), "改密必须级联删会话");
...
fn pa_session_single_per_account_and_independent_from_admin() {
    ...
    let err = s.apply(&Command::PaSessionLogin {
        username: "alice".to_string(),
    ...
    // alice 登出后可重登
    s.apply(&Command::PaSessionLogout { username: "alice".to_string() }, 1_400).unwrap();
...
fn pa_session_heartbeat_extends_expiry() {
    ...
    s.apply(&Command::PaSessionHeartbeat {
        username: "alice".to_string(),
        expires_at: Some(20_000),
    }, 2_000).unwrap();
    assert_eq!(s.get_pa_session("alice").unwrap().unwrap().expires_at, Some(20_000));
    ...
    // 无会话 → 错误
    s.apply(&Command::PaSessionLogout { username: "alice".to_string() }, 4_000).unwrap();
    let err = s.apply(&Command::PaSessionHeartbeat {
        username: "alice".to_string(),
        expires_at: Some(1),
    }, 5_000).unwrap_err();
...
fn project_delete_cascades_pa_accounts_and_sessions() {
    let mut s = setup_project("alpha");
    let mut other = sm();
    other.apply(&Command::ProjectCreate {
        name: "beta".to_string(),
        operator: String::new(),
    }, 1_000).unwrap();
    ...
    s.apply(&Command::ProjectDelete {
        id: ProjectId("alpha".to_string()),
        operator: String::new(),
    }, 3_000).unwrap();

    assert!(s.get_project_admin("alice").unwrap().is_none(), "删项目必须级联删 PA 账号");
    assert!(s.get_project_admin("bob").unwrap().is_none());
    assert!(s.get_pa_session("alice").unwrap().is_none(), "删项目必须级联删 PA 会话");
...
fn publish_operator_recorded_in_version() {
    ...
    s.apply(&Command::PublishStructure {
        project: ProjectId("alpha".to_string()),
        comment: "init".to_string(),
        request_id: "req-test".to_string(),
        operator: "pa:alice".to_string(),
    }, 1_600).unwrap();
    s.apply(&Command::DraftUpdate {
        project: ProjectId("alpha".to_string()),
    ...
    let versions = s.version_history(&ProjectId("alpha".to_string()), &BranchName("dev".to_string())).unwrap();
...
fn publish_operator_defaults_to_admin() {
    ...
    let versions = s.version_history(&ProjectId("alpha".to_string()), &BranchName("dev".to_string())).unwrap();
    assert_eq!(versions.last().unwrap().operator, "admin", "空 operator 应回退为 admin");
}

#[test]
fn operator_field_serde_backward_compatible() {
    // 旧格式 JSON（无 operator 字段）必须能反序列化（Raft 日志重放兼容）
    let old_publish = r#"{"Publish":{"project":"alpha","branch":"dev","comment":"c","request_id":"r1"}}"#;
