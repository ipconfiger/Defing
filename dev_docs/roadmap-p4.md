...
| 阶段 | 内容 | 关键代码面 | 验收 | 估时 |
|------|------|-----------|------|------|
| **R0 设计先行** | RBAC 模型文档（角色集、权限粒度=项目级/分支级/item 级、审批流归属、多租户边界）；决策：是否本期做多项目（建议：本期做角色+分支级，多项目做二期） | dev_docs/（新增 `design/rbac.md`） | 设计评审通过；决策记录落档 | 2 天 |
...
| **E5 成熟度证据** | 压测基准归档（bench.sh 结果入 docs）、k6/ghz 场景脚本、生产案例文档模板 | dev_docs/benchmarks、scripts | 基准数字可复现；CI 基准门槛（可选） | 2–3 天 |
...
| M-P4-4（W12 末） | ✅ **G4 灰度管理面+UI 完成** + G5 百分比放量 + R4 审批流 + E4/E5 Helm+基准 | M-P4-3 | G4：api-surface 灰度断言 12 项全过 + UI 实测；G5/R4/E4/E5 待续 |
