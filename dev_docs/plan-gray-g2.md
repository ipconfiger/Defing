...
> **状态：✅ 全部完成（2025-08-16）**——13 任务全闭环，workspace 测试全绿、clippy 0 警告，提交后 roadmap/design 同步标记。
...
| 7 | `resolve_version`/`rule_matches`/`fnv1a_hash` 读路径纯函数（固定求值次序 labels→IP→percent；无身份不进灰度） | dsh-core/src/state.rs | 编译 | ✅（另含 ip_in_cidr 与规则校验） |
| 8 | apply_publish_structure：灰度活跃时灰度快照同步 bump（分配不同号）+ 事件 | dsh-core/src/state.rs | 编译 | ✅（stable_next=max+1、gray_next=stable_next+1，T6 覆盖） |
| 9 | prune_versions：保留条件加 `\|\| no == gray_seq 指向`（Q5） | dsh-core/src/state.rs | 编译 | ✅（gray-snap/ 前缀独立于 v/ 天然保留，T8 实测） |
| 10 | 事件/版本记录加 `gray: bool`（serde default，不新增 EventType 枚举值，Q3） | dsh-core/src/model.rs | 编译 | ✅（PublishEvent + VersionRecord；watch 重放还原 gray 标记） |
| 11 | 测试 T1-T8（灰度发布/解析三路/转正/下量/幂等/结构发布×gray/无身份不进灰度/保留策略） | dsh-core/tests/state_machine.rs | 全绿 | ✅（T1-T8 + 错误路径 T5b，共 47 用例） |
| 12 | 全量测试 + e2e + clippy/fmt | 命令行 | 达标 | ✅（workspace 31 套件全绿；clippy 0 警告；fmt 通过） |
| 13 | 更新 gray-release.md / roadmap-p4.md 状态 | docs | 标记 G2 完成 | ✅ |

> 额外收尾（G2 完整性，超出计划表但属同一阶段）：`rewrap_deks` 覆盖灰度快照
> （含 Secret 的灰度快照在 KEK 轮换后可解密）；api 重放合成事件还原 `gray` 标记（Q3/Q4 重放保真）。
