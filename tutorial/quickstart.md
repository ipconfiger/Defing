...
> `--dev-single` 为单机内存模式（重启即清空），适合联调；生产部署见 [01 部署与启动](01-install/)。

## 2. 登录 Admin UI

浏览器打开 `http://127.0.0.1:8384/admin`：

![登录页](assets/images/01-login.png)
...
令牌在项目页「访问令牌」页签创建（详见 [07 访问令牌与 SDK](07-tokens/) 与 [08 构建脚本取值](08-curl/)）。

## 下一步

- [01 部署与启动](01-install/)：单机 / 集群 / 参数详解
- [02 项目与分支](02-project/)：项目与分支管理
- [04 草稿与发布](04-draft/)：编辑、校验、版本与回滚
- [05 灰度发布](05-gray/)：灰度上线与回滚
