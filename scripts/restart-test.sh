...
for id in 1 2 3; do
  ...
  $BIN --node-id $id --http-addr 127.0.0.1:861$id --raft-addr 127.0.0.1:871$id --grpc-addr 127.0.0.1:881$id --data-dir $W/n$id --admin-password admin123 --allow-no-master-key --join-token demo --raft-token demo $FLAG >$W/n$id.log 2>&1 &
done
sleep 3
...
$BIN --node-id 1 --http-addr 127.0.0.1:8611 --raft-addr 127.0.0.1:8711 --grpc-addr 127.0.0.1:8811 --data-dir $W/n1 --admin-password admin123 --allow-no-master-key --join-token demo --raft-token demo >$W/n1r.log 2>&1 &
sleep 10
